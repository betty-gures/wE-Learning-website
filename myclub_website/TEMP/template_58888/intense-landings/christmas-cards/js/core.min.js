 ...
 */
function getSnow(t,s){var i=document.getElementById("canv"),o=i.getContext("2d"),d=i.width=window.innerWidth,r=i.height=window.innerHeight;!function(){for(var t,n=[],h=1,a=20,i=0;i<300;++i)(t=new e).y=Math.random()*(r+50),t.x=Math.random()*d,t.t=Math.random()*(2*Math.PI),t.sz=100/(10+100*Math.random())*1.3,t.sp=.01*Math.pow(.8*t.sz,2)*1,t.sp=t.sp<1?1:t.sp,n.push(t);function e(){this.draw=function(){this.g=o.createRadialGradient(this.x,this.y,0,this.x,this.y,this.sz),this.g.addColorStop(0,"hsla(255,255%,255%,1)"),this.g.addColorStop(1,"hsla(255,255%,255%,0)"),o.moveTo(this.x,this.y),o.fillStyle=this.g,o.beginPath(),o.arc(this.x,this.y,this.sz,0,2*Math.PI,!0),o.fill()}}!function t(){window.requestAnimationFrame(t);o.clearRect(0,0,d,r);o.fillStyle=s||"hsla(242, 95%, 3%, 1)";o.fillRect(0,0,d,r);o.fill();for(var i=0;i<n.length;++i)f=n[i],f.t+=.05,f.t=f.t>=2*Math.PI?0:f.t,f.y+=f.sp,f.x+=Math.sin(f.t*h)*(.3*f.sz),f.y>r+50&&(f.y=-10-Math.random()*a),f.x>d+a&&(f.x=-a),f.x<-a&&(f.x=d+a),f.draw()}()}(),window.addEventListener("resize",function(){i.width=d=window.innerWidth,i.height=r=window.innerHeight},!1)}